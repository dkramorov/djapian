...
class ResultSet(object):
    ...
    def get_parsed_query_terms(self, with_stopper=True):
        if with_stopper:
    ...
    def get_spies(self):
        """Получаем словарь spy по фасетам"""
        self._get_mset()
        return self._spies or {}
    ...
    def _get_mset(self):
        if self._mset is None:
            self._mset, self._query, self._query_parser, self._spies = self._indexer._do_search(
                self._query_str,
                self._offset,
                self._limit,
                self._order_by,
    ...
    def _parse_results(self):
        ...
        for match in self._mset:
            ...
            model = apps.get_model(*model.decode('utf-8').split('.'))
    ...
    def __unicode__(self):
        return u"<ResultSet: query=%s>" % force_text(self._query_str)
